...
Notice that these n-grams can contain stopwords, so it is often a good idea to filter out any results that include at least one. It is important, however, to mine n-grams before removing stopwords. Otherwise, the cleaning will alter the sequences of words and provide incorrect results. In practice, we can mine text for all n-grams with lengths between some specified minimum and maximum, but we start here by extracting the bi-grams from *A Tale of Two Cities*.
...
When we compare the results of TF-IDF here to the results with single word tokenization, we can see the emphasis on complete character names and, in some cases, actions of characters. By including stopwords in this, we also have the opportunity to see words that would never be significant on their own, but can be important as part of a sequence.
...
After running the model, we can obtain the probabilities of each word belonging to each topic from the beta matrix in tidy. This is the conditional probability $p(w_n|z_n,\beta)$ from the graphical LDA model. It will return a row for every word and topic and the associated probability. Let's graph the 7 most likely words in each topic.
