...
It is important to note that the words envious, enviously, and enviousness do not appear in *A Tale of Two Cities*, so there will be no duplicate sentiments for each word. The book begins with 137,388 words but 91,188 stop words were removed leaving 46,200 words for analysis.
...
First, we can look at the probabilities of each word belonging to each topic. After converting the LDA results to a tidy data frame, there will be a row for each topic and term and its probability. Let's convert this into showing the 7 most likely words in each topic and graph it.
...
```{r}
## Run LDA using several different number of topics
...
## Create a table of which authors were in which topic
books.docs.optimal %>% 
   group_by(document) %>%  # Group by each book
