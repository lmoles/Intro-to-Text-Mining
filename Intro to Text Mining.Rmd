...
To run the LDA model from the topicmodels package, we will also have to select a k for the number of topics (authors). Let's try 5 and set a seed for reproducibility.
...
First, we can look at the probabilities of each word belonging to each topic. After converting the LDA results to a tidy data frame, there will be a row for each topic and term and its probability. Let's graph the 7 most likely words in each topic.
